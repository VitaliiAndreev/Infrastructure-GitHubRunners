...
1. Inline guard installs/imports `Common.PowerShell` (the only install
   logic that cannot use `Invoke-ModuleInstall` — the function isn't
   available until the module is loaded).
2. `Invoke-ModuleInstall` (from `Common.PowerShell`) installs and imports
...
3. `Initialize-InfrastructureVault` called with:
   ...
   - `-Validate`: scriptblock that calls `Assert-RequiredProperties` (from
     `Common.PowerShell`) on each entry — validation runs inside
...
sequenceDiagram
    ...
    SS->>PSG: install Common.PowerShell (inline guard)
...
1. Imports `Common.PowerShell` (install guard, same inline pattern as
...
5. Calls `Assert-RequiredProperties` (from `Common.PowerShell`) on each
...
1. Open an SSH connection as `u-runner-deploy` using SSH.NET directly
   ...
   `Common.PowerShell`) for all remote commands. Posh-SSH is
